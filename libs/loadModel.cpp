 ...
 *
 * Initializes the file paths for the model, configuration, and class labels.
 *
 * @param modelPath Path to the model file.
 * @param configPath Path to the configuration file.
 * @param classesPath Path to the file containing class labels.
 */
loadModel::loadModel(const std::string& modelPath,
...
bool loadModel::loadFromFile() {
  ...
  if (net.empty()) {
    ...
    errorMsg << "Failed to load the neural network model from the path: "
             << model_file_path;
    throw std::runtime_error(errorMsg.str());
  }
  std::cout << "Model has been successfully loaded from: " << model_file_path
            << std::endl;
