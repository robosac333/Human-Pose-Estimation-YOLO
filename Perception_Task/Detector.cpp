...
std::vector<cv::Rect> Detector::detectHumans(const cv::Mat& inputImage) {
    std::cout << "Creating blob from input image. Image dimensions: " << inputImage.size() << std::endl;
     ...
     *          - Crop: false
     */
    cv::Mat imageBlob = cv::dnn::blobFromImage(inputImage, 1.0 / 255.0, 
                                              cv::Size(416, 416), 
                                              cv::Scalar(0, 0, 0), 
                                              true, false);
    ...
    std::cout << "Total number of layers in the network: " << allLayerNames.size() << std::endl;

    /**
     * @brief Process network output
     * @details Collects output from unconnected layers and processes detection results
     */
    ...
    for (const auto& outputData : networkOutputs) {
        for (int rowIdx = 0; rowIdx < outputData.rows; ++rowIdx) {
             ...
             */
            cv::Mat probabilityScores = outputData.row(rowIdx).colRange(5, outputData.cols);
            cv::Point maxScorePosition;
            double maxScoreValue;
            cv::minMaxLoc(probabilityScores, nullptr, &maxScoreValue, nullptr, &maxScorePosition);

            // Process detections with high confidence (>0.5) that are classified as people
            if (maxScoreValue > 0.5 && classLabels[maxScorePosition.x] == "person") {
                // Calculate bounding box coordinates
                int xCenter = static_cast<int>(outputData.at<float>(rowIdx, 0) * inputImage.cols);
                int yCenter = static_cast<int>(outputData.at<float>(rowIdx, 1) * inputImage.rows);
                int boxWidth = static_cast<int>(outputData.at<float>(rowIdx, 2) * inputImage.cols);
                int boxHeight = static_cast<int>(outputData.at<float>(rowIdx, 3) * inputImage.rows);
